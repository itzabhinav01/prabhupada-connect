...
for rkey, bkey, ref, trans, purp in rows:
    for fld, text in [('Translation', trans), ('Purports', purp)]:
        if not text:
            continue
        # Match 'Thus end the Bhaktivedanta purports' followed by anything up to the closing period and optional quote
        m = re.search(r'Thus end the Bhaktivedanta purports\b.*?[\.\?]["\']?', text, re.IGNORECASE | re.DOTALL)
        if m:
            ...
            if tail_clean and any(k in tail_clean for k in ['Chapter', '—', ':', 'Canto', 'Ādi', 'Adi', 'Madhya', 'Antya', 'SB ']):
                ...
                if count <= 15 or count >= 100:
                    ...
                    print(f"    END: ...{end_snippet[-70:]}")
                    print(f"    LEAK ({len(tail)} chars): {tail[:100]}...")
                    print("-" * 60)

print(f"\nTOTAL LEAKS FOUND ACROSS CORPUS: {count}")
conn.close()
